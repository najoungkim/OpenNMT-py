...
python -u ../train.py -data $DATA_PATH/$EXAMPLES -save_model $SAVE_PATH/${SAVE_NAME}/s$SEED \
        ...
        -world_size 1 -gpu_ranks 0 -seed $SEED --log_file ../logs/${EXAMPLES}_smaller_s${SEED}.runlog

#mkdir ../preds

#python ../translate.py -model $SAVE_PATH/${EXAMPLES}/s${SEED}_step_2500.pt \
#                    -src $DATA_PATH/gen_source.txt \
#                    -tgt $DATA_PATH/gen_target.txt \
#                    -output ../preds/gen_pred_${EXAMPLES}_smaller_s${SEED}.txt \
#                    -replace_unk -verbose -shard_size 0 \
#                    -gpu 0 -batch_size 128

#paste $DATA_PATH/gen_source.txt $DATA_PATH/gen_target.txt ../preds/gen_pred_${EXAMPLES}_smaller_s${SEED}.txt > ../preds/gen_pred_${EXAMPLES}_smaller_s${SEED}.tsv


#python ../translate.py -model $SAVE_PATH/1_example/s${SEED}_step_2500.pt \
#                    -src $DATA_PATH/test_source.txt \
#                    -tgt $DATA_PATH/test_target.txt \
#                    -output ../preds/test_pred_1_example_s1.txt \
#                    -replace_unk -verbose -shard_size 0 \
#                    -gpu 0 -batch_size 128

#paste $DATA_PATH/test_source.txt $DATA_PATH/test_target.txt ../preds/test_pred_1_example_s1.txt > ../preds/test_pred_1_example_s1.tsv
